...
def load(data_dir, subset='train'):
    ...
    if subset=='train':
        trainx, trainy, trainh = unpickle(os.path.join(data_dir, 'celeba_train_set_128.p'))
        return trainx, trainy, trainh
    elif subset=='test':
        testx, testy, testh = unpickle(os.path.join(data_dir, 'celeba_test_set_128.p'))
        return testx, testy, testh
    elif subset=='sample':
        samplex, sampley, sampleh = unpickle(os.path.join(data_dir, 'celeba_sample_set_128.p'))
...
class DataLoader(object):
    ...
    def __init__(self, data_dir, subset, batch_size, rng=None, shuffle=False, return_labels=False):
        ...
        self.data, self.labels, self.h = load(os.path.join(data_dir,'celeba'), subset=subset)
        if subset == 'sample':
